...
logging.basicConfig(filename='logs/GridSearchTuner.log', level=logging.DEBUG,filemode='w', format='%(name)s - %(levelname)s - %(message)s')
...
def main():
    ...
    callbacks=[autotvm.callback.log_to_file("results/tvm_GridSearchTuner.json")]
    )
    end = time.time()

    logging.info("Elpased time = {}".format(end-start))

    with autotvm.apply_history_best("results/tvm_GridSearchTuner.json"):
