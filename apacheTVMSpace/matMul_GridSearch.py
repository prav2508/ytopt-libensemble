...
def main():
    ...
    measure_option = autotvm.measure_option(builder="local", 
                                            runner=autotvm.LocalRunner(number=1, repeat=1, timeout=200),
                                            )
    ...
    callbacks=[autotvm.callback.log_to_file("tvm_GridSearchTuner.log")]
    )

    with autotvm.apply_history_best("tvm_GridSearchTuner.log"):
